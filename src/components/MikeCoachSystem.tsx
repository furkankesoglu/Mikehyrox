...
export default function MikeCoachSystem(){
 ...
 const activeProgram=useMemo(()=>programs.filter(p=>p.status==="approved").sort((a,b)=>b.weekNumber-a.weekNumber)[0]||null,[programs]);
 const activeWeek=activeProgram?.weekNumber||1;
 const activeDays=useMemo(()=>activeProgram?programToWeekDays(activeProgram):week1Program,[activeProgram]);
 useEffect(()=>{if(!activeDays.some(d=>d.key===dayKey))setDayKey(activeDays[0]?.key||"monday")},[activeDays,dayKey]);
 const day=activeDays.find(d=>d.key===dayKey)||activeDays[0];
 const draftId=(exerciseId:string)=>`w${activeWeek}:${exerciseId}`;
 const dayLogs=useMemo(()=>Object.fromEntries((day?.exercises||[]).map(e=>[e.id,drafts[draftId(e.id)]||emptyLog])),[day,drafts,activeWeek]);
 ...
 async function saveWorkout(){if(!day)return;const hasData=day.exercises.some(e=>(dayLogs[e.id]?.status||"pending")!=="pending");if(!hasData){setMessage("Önce en az bir hareketin durumunu gir.");return}const record:SavedWorkout={id:`w${activeWeek}-${day.key}-${Date.now()}`,week:activeWeek,date:new Date().toISOString().slice(0,10),dayKey:day.key,dayLabel:day.label,theme:day.theme,savedAt:new Date().toISOString(),exercises:day.exercises.map(e=>({id:e.id,title:e.title,target:e.target,log:dayLogs[e.id]}))};const next=[record,...history.filter(r=>workoutKey(r)!==workoutKey(record))];setHistory(next);localStorage.setItem(HISTORY_KEY,JSON.stringify(next));setMessage(`${day.label} antrenmanı kaydediliyor...`);const synced=await syncRecord("workout",record);setMessage(`${day.label} antrenmanı ${synced?"buluta ve cihaza":"cihaza"} işlendi.`);setTimeout(()=>setMessage(""),2800)}
