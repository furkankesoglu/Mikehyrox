...
const AUTH_KEY="mike-authenticated",DRAFT_KEY="mike-workout-drafts-v4",HISTORY_KEY="mike-workout-history-v1",CHECKIN_KEY="mike-checkins-v3",PROGRAM_KEY="mike-approved-programs-v1";
...
const dayMeta:Record<string,{key:string;short:string;label:string}>={
...
function programToWeekDays(program:Program):WeekDay[]{
 return program.days.map((item,dayIndex)=>{
  const meta=dayMeta[item.day.toLocaleLowerCase("tr-TR")]||{key:`day-${dayIndex+1}`,short:`G${dayIndex+1}`,label:item.day};
  return {key:meta.key,short:meta.short,label:meta.label,theme:item.theme,duration:"Planlanan seans",exercises:item.exercises.map((exercise,index)=>({id:`w${program.weekNumber}-${meta.key}-${index}-${slug(exercise.title)}`,category:"HYROX PLAN",title:exercise.title,target:exercise.prescription,note:exercise.safetyNote}))};
 });
}
...
export default function MikeCoachSystem(){
 ...
 useEffect(()=>{
  ...
  try{setDrafts(JSON.parse(localStorage.getItem(DRAFT_KEY)||"{}"));localHistory=JSON.parse(localStorage.getItem(HISTORY_KEY)||"[]");localCheckins=JSON.parse(localStorage.getItem(CHECKIN_KEY)||"[]");localPrograms=JSON.parse(localStorage.getItem(PROGRAM_KEY)||"[]");setHistory(localHistory);setCheckins(localCheckins);setPrograms(localPrograms)}catch{}
  setReady(true);
  fetch("/api/sync",{cache:"no-store"}).then(async response=>{
   const data:CloudPayload=await response.json();if(!data.configured){setCloud("local");return}if(!response.ok||data.error)throw new Error(data.error||"Bulut okunamadı");
   ...
   const mergedHistory=mergeBy(localHistory,cloudHistory,workoutKey),mergedCheckins=mergeBy(localCheckins,cloudCheckins,x=>x.date),mergedPrograms=mergeBy(localPrograms,cloudPrograms,programKey);
   setHistory(mergedHistory);setCheckins(mergedCheckins);setPrograms(mergedPrograms);localStorage.setItem(HISTORY_KEY,JSON.stringify(mergedHistory));localStorage.setItem(CHECKIN_KEY,JSON.stringify(mergedCheckins));localStorage.setItem(PROGRAM_KEY,JSON.stringify(mergedPrograms));
   const cloudWorkoutKeys=new Set(cloudHistory.map(workoutKey));await Promise.all(localHistory.filter(x=>!cloudWorkoutKeys.has(workoutKey(x))).map(data=>fetch("/api/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"workout",data})})));setCloud("synced");
 ...
 const activeProgram=approvedPrograms[0]||null;
 const activeWeek=activeProgram?.weekNumber||1;
 useEffect(()=>{if(selectedWeek===null)setSelectedWeek(activeWeek)},[activeWeek,selectedWeek]);
 const viewedWeek=selectedWeek??activeWeek;
 const viewedProgram=approvedPrograms.find(p=>p.weekNumber===viewedWeek)||null;
 ...
 const day=viewedDays.find(d=>d.key===dayKey)||viewedDays[0];
 const draftId=(exerciseId:string)=>`w${viewedWeek}:${exerciseId}`;
 ...
 async function saveWorkout(){if(!day)return;const hasData=day.exercises.some(e=>(dayLogs[e.id]?.status||"pending")!=="pending");if(!hasData){setMessage("Önce en az bir hareketin durumunu gir.");return}const record:SavedWorkout={id:`w${viewedWeek}-${day.key}-${Date.now()}`,week:viewedWeek,date:new Date().toISOString().slice(0,10),dayKey:day.key,dayLabel:day.label,theme:day.theme,savedAt:new Date().toISOString(),exercises:day.exercises.map(e=>({id:e.id,title:e.title,target:e.target,log:dayLogs[e.id]}))};const next=[record,...history.filter(r=>workoutKey(r)!==workoutKey(record))];setHistory(next);localStorage.setItem(HISTORY_KEY,JSON.stringify(next));setMessage(`${day.label} antrenmanı kaydediliyor...`);const synced=await syncRecord("workout",record);setMessage(`${day.label} antrenmanı ${synced?"buluta ve cihaza":"cihaza"} işlendi.`);setTimeout(()=>setMessage(""),2800)}
