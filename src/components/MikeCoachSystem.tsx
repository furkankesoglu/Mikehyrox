...
export default function MikeCoachSystem(){
  ...
  const [checkin,setCheckin]=useState<Checkin>(emptyCheckin),[message,setMessage]=useState(""),[pending,setPending]=useState<Program|null>(null);
  ...
  useEffect(()=>{
    ...
    try{
      ...
      setPrograms(JSON.parse(localStorage.getItem(PROGRAM_KEY)||"[]"));
    }catch{}
    setReady(true);
    ...
    fetch("/api/sync",{cache:"no-store"})
      .then(async response=>{
        ...
        const cloudHistory:SavedWorkout[]=(data.workouts||[]).map(row=>({
          id:text(row.id),week:Number(row.week_number)||1,date:text(row.workout_date),dayKey:text(row.day_key),dayLabel:text(row.day_label),theme:text(row.theme),savedAt:text(row.updated_at||row.created_at),exercises:(row.exercises as SavedWorkout["exercises"])||[]
        }));
        const cloudCheckins:Checkin[]=(data.checkins||[]).map(row=>({
          date:text(row.checkin_date),weight:text(row.weight_kg),waist:text(row.waist_cm),sleep:text(row.sleep_hours),sleepQuality:text(row.sleep_quality),energy:text(row.energy),backPain:text(row.back_pain)
        }));
        const cloudPrograms:Program[]=(data.programs||[]).map(row=>({
          id:text(row.id),weekNumber:Number(row.week_number),title:text(row.title),rationale:text(row.rationale),status:text(row.status) as Program["status"],createdAt:text(row.created_at),days:(row.program as Program["days"])||[]
        }));

        setHistory(cloudHistory);setCheckins(cloudCheckins);setPrograms(cloudPrograms);
        localStorage.setItem(HISTORY_KEY,JSON.stringify(cloudHistory));
        localStorage.setItem(CHECKIN_KEY,JSON.stringify(cloudCheckins));
        localStorage.setItem(PROGRAM_KEY,JSON.stringify(cloudPrograms));
        setCloud("synced");
      })
  ...
  async function syncRecord(type:"workout"|"checkin"|"program",data:SavedWorkout|Checkin|Program){
    try{
      const response=await fetch("/api/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type,data})});
      const result=await response.json();
      if(!result.configured){setCloud("local");return false;}
      if(!response.ok)throw new Error(result.error||"Bulut kaydı başarısız");
      setCloud("synced");return true;
    }catch{setCloud("error");return false;}
  }

  async function saveWorkout(){
    const hasData=day.exercises.some(e=>(dayLogs[e.id]?.status||"pending")!=="pending");
    if(!hasData){setMessage("Önce en az bir hareketin durumunu gir.");return}
    const record:SavedWorkout={id:`w1-${day.key}-${Date.now()}`,week:1,date:new Date().toISOString().slice(0,10),dayKey:day.key,dayLabel:day.label,theme:day.theme,savedAt:new Date().toISOString(),exercises:day.exercises.map(e=>({id:e.id,title:e.title,target:e.target,log:dayLogs[e.id]}))};
    const next=[record,...history.filter(r=>!(r.week===1&&r.dayKey===day.key&&r.date===record.date))];
    setHistory(next);localStorage.setItem(HISTORY_KEY,JSON.stringify(next));
    setMessage(`${day.label} antrenmanı kaydediliyor...`);
    const synced=await syncRecord("workout",record);
    setMessage(`${day.label} antrenmanı ${synced?"buluta ve cihaza":"cihaza"} işlendi.`);setTimeout(()=>setMessage(""),2800);
  }

  async function saveCheckin(e:FormEvent){
    e.preventDefault();const next=[checkin,...checkins.filter(c=>c.date!==checkin.date)];setCheckins(next);localStorage.setItem(CHECKIN_KEY,JSON.stringify(next));
    setMessage("Check-in kaydediliyor...");const synced=await syncRecord("checkin",checkin);setMessage(`Check-in ${synced?"buluta ve cihaza":"cihaza"} kaydedildi.`);setTimeout(()=>setMessage(""),2400);
  }
  ...
  async function approve(){
    if(!pending)return;const approved={...pending,status:"approved" as const};const next=[approved,...programs.filter(p=>!(p.weekNumber===approved.weekNumber&&p.status==="approved"))];setPrograms(next);localStorage.setItem(PROGRAM_KEY,JSON.stringify(next));setPending(null);
    setMessage(`${approved.weekNumber}. hafta programı kaydediliyor...`);const synced=await syncRecord("program",approved);setMessage(`${approved.weekNumber}. hafta programı ${synced?"buluta ve sisteme":"cihaza"} eklendi.`);
  }
